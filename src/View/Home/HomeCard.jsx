...
const HomeCard = () => {
  ...
  const getmanagerData = async () => {
    const res = await HttpClient.requestData(
      "view-all-esgUser", "GET", {}
    );
    console.log("resCat", res);
    if (res && res?.status) {
      setCompanyAdmin(prev => ({ ...prev, managerLength: res?.data?.length })
  ...
  const gethrData = async () => {
    ...
    console.log("resCat", res);
    if (res && res?.status) {
      setCompanyAdmin(prev => ({ ...prev, hrLength: res?.data?.length })
  ...
  const getEmployeeData = async () => {
    ...
    console.log("resCat", res);
    if (res && res?.status) {
      setCompanyAdmin(prev => ({ ...prev, employeeLength: res?.data?.length })
      );
    } else {
      // toast(res?.message);
    }
    ...
    <>
      <div className="HomeTop">
        <div className="row">
          ...
          <div className="col-md-4 col-12">
            <div className="HomeCard">
              <h5 className="HomeCardHead">Total ESG-Users :</h5>
              <p className="HomeCardPara">
                <span
                  style={{
                    color: item[1],
